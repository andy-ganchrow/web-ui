...
describe('new-simulation-state', () => {
  describe('initializeSimulationState', () => {
    ...
    it('should initialize state with two interventions of increasing input.Intensity', () => {
      const state = initializeSimulationState(
        createMockRegions(),
        createMockInterventions({
          GathRestrict10: {
    ...
    it('should initialize state with two interventions on the different dates, first overriding the second', () => {
      // GathRestrict10 and GathRestrict10 are the same input.Intensity, so our periods have not changed. Just use
      // the first period.
      const state = initializeSimulationState(
        createMockRegions(),
        createMockInterventions({
